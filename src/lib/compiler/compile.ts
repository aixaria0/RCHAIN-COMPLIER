...
export const SCENARIOS: ScenarioMeta[] = [
  {
    ...
    summary:
      "Bob trades AliceCoin on shard A for GBP on shard B via prepare → prepareReceive → commit / commit.",
    killer: true,
  },
  {
    id: "exchange-abort",
...
export const MUTATIONS: MutationMeta[] = [
  {
    ...
    applies: ["exchange-commit", "exchange-abort", "hello-rho", "cap-payment"],
  },
  {
    id: "node-c-lied",
  ...
  },
  {
    ...
    applies: ["exchange-commit", "exchange-abort", "cap-payment"],
  },
  {
    id: "force-abort",
  ...
  },
  {
    ...
    applies: ["exchange-commit", "hello-rho", "cap-payment"],
  },
];
...
export interface EventEnvelope {
  ...
  qlfDigest: string | null;
  rholangSourceHash: string | null;
  normalizedProcess: string | null;
  executionTraceHash: string | null;
  deployId: string | null;
  blockHash: string | null;
  nodeObservations: string[];
...
export interface Reality {
  ...
  mutation: MutationId;
  qos: QosEvent;
  qlf: QlfCertificate | null;
  rholangSource: string | null;
  execution: Execution | null;
  exchange: ExchangeWorld | null;
  deploys: Deploy[];
  blocks: Block[];
  observations: NodeObservation[];
  cross: CrossNodeReport | null;
  casper: CasperEvidence | null;
...
function envelope(partial: Omit<EventEnvelope, "hash" | "payloadHash" | "prevHash"> & { prev: string | null }): EventEnvelope {
  ...
  const hash = hexPrefixed(digest(["env", payloadHash, prev ?? "genesis"]));
...
export function compile(scenario: ScenarioId, mutation: MutationId): Reality {
  ...
  if (authorized) {
    if (isExchange) {
      ...
      if (abortScenario) {
        // remote refuses — no link used / abort local
      ...
      w.conserved = true;
      exchange = w;
      const verbs = abortScenario
        ? ["deposit", "prepare", "abort"]
        : ["deposit", "prepare", "prepareReceive", "commit", "commit"];
  ...
  const qlf = authorized
    ? certify(phasesFromComms(execution?.comms ?? 0), true)
    : certify(["+"], false);
  ...
  const obsOpts = {
    ...
    duplicateProposer:
      mutation === "dup-validator" ? { from: "C", onto: "B" } : undefined,
  };
  const observations = primary
    ? observeBlock(primary, obsOpts)
    : [];
  ...
  if (execution) {
    ...
    const replaySteps: ReductionStep[] = execution.steps.map((s, i) =>
      tamper && s.rule === "COMM" && i === execution.steps.findIndex((x) => x.rule === "COMM")
        ? { ...s, continuation: "receive continuation′", description: s.description + " [TAMPERED]" }
        : s,
    );
    const expected = execution.stateHash;
    const observed = tamper
      ? hexPrefixed(digest(["tamper", expected]))
      : expected;
    const first = tamper
      ? {
...
function linkEnvelopes(args: {
  ...
  add({
    ...
    fields: [
      { source: "QuantumOS", field: "event_id", value: args.qos.eventId },
      { source: "QuantumOS", field: "peer", value: args.qos.peer },
      { source: "QuantumOS", field: "capability", value: args.qos.capability ?? "ABSENT" },
      { source: "QuantumOS", field: "lemma", value: args.qos.lemma },
      { source: "QuantumOS", field: "room", value: args.qos.room },
    ],
  });
  ...
  add({
    ...
    fields: [
      { source: "QLF", field: "phase_string", value: args.qlf.phaseString || "(empty)" },
      { source: "QLF", field: "count(+)", value: String(args.qlf.countPos) },
      { source: "QLF", field: "count(-)", value: String(args.qlf.countNeg) },
      { source: "QLF", field: "spectral_gap", value: String(args.qlf.spectralGap) },
      { source: "QLF", field: "symmetric", value: String(args.qlf.symmetric) },
      { source: "QLF", field: "spectral_form", value: args.qlf.spectralForm },
    ],
  });
  ...
  if (args.rholangSource && args.execution) {
    add({
      ...
      fields: [
        { source: "RholangProcess", field: "source_digest", value: hexPrefixed(digest(["src", args.rholangSource])) },
        { source: "RholangProcess", field: "normalized", value: args.execution.normalized },
        { source: "RholangProcess", field: "comms", value: String(args.execution.comms) },
      ],
    });
    add({
  ...
  if (args.deploys[0] && args.blocks[0]) {
    ...
    add({
      ...
      fields: [
        { source: "FinalizedBlockEvidence", field: "block_height", value: String(b.height) },
        { source: "FinalizedBlockEvidence", field: "block_hash", value: b.hash },
        { source: "FinalizedBlockEvidence", field: "parent_hash", value: b.parentHash },
        { source: "FinalizedBlockEvidence", field: "proposer", value: b.proposer },
        { source: "FinalizedBlockEvidence", field: "post_state_hash", value: b.postStateHash },
      ],
    });
  }
  ...
  if (args.lattice) {
    add({
      ...
      fields: [
        { source: "SovereignLattice", field: "N", value: String(args.lattice.n) },
        { source: "SovereignLattice", field: "f", value: String(args.lattice.f) },
        { source: "SovereignLattice", field: "Q", value: String(args.lattice.quorum) },
        { source: "SovereignLattice", field: "prepared_certificate", value: String(args.lattice.preparedCertificate) },
        { source: "SovereignLattice", field: "committed_certificate", value: String(args.lattice.committedCertificate) },
      ],
    });
  }
  ...
  add({
    ...
    fields: [
      { source: "VerificationReport", field: "envelopes", value: String(out.length + 1) },
      { source: "VerificationReport", field: "linked", value: "sha256 envelope chain" },
    ],
  });
...
function evaluate(args: {
  ...
  invariants.push({
    ...
    severity: "CRITICAL",
    detail: args.authorized
      ? `${args.qos.capability} authorizes lemma ${args.qos.lemma}.`
      : "No capability present; QuantumOS refuses to originate a deployable event.",
    evidence: [
      { source: "QuantumOS", field: "capability", value: args.qos.capability ?? "ABSENT" },
      { source: "QuantumOS", field: "lemma", value: args.qos.lemma },
    ],
  ...
  if (args.execution) {
    invariants.push({
      ...
      evidence: [
        { source: "ExecutionTrace", field: "comms", value: String(args.execution.comms) },
        { source: "ExecutionTrace", field: "state_hash", value: args.execution.stateHash },
      ],
    });
  } else {
    invariants.push({
      ...
      evidence: [{ source: "ExecutionTrace", field: "available", value: "false" }],
    });
  }
  ...
  if (args.replay) {
    invariants.push({
      ...
      severity: "CRITICAL",
      detail: args.replay.match
        ? "Replayed reduction produces the same state hash as the block post-state."
        : `Divergence at reduction step ${args.replay.firstDivergence?.step ?? "?"}.`,
      evidence: [
        { source: "Replay", field: "expected_state_hash", value: args.replay.expectedStateHash },
        { source: "Replay", field: "observed_state_hash", value: args.replay.observedStateHash },
      ],
    });
  }

  if (args.cross) {
    const heightOut = args.observations.find(
      (o) => o.reachable && o.lastFinalizedBlockNumber !== args.cross!.commonHeight,
    );
    const hashOut = args.observations.find(
      (o) => o.reachable && o.blockHash !== args.cross!.commonHash,
    );
    invariants.push({
      id: "I-01",
      layer: "sentinel",
    ...
    invariants.push({
      ...
      evidence: [{ source: "RNodeObservation", field: "available", value: "false" }],
    });
  }
  ...
  invariants.push({
    ...
    severity: "CRITICAL",
    detail: dup
      ? `Duplicate proposer ${dup.proposer}.`
      : "Distinct proposer identities on a single network and shard.",
    evidence: args.observations.map((o) => ({
  ...
  if (args.lattice) {
    invariants.push({
      ...
      evidence: [
        { source: "SovereignLattice", field: "committed_count", value: String(args.lattice.committedCount) },
        { source: "SovereignLattice", field: "quorum", value: String(args.lattice.quorum) },
      ],
    });
    invariants.push({
      id: "L-02",
      ...
      severity: "CRITICAL",
      detail: args.lattice.conflictingPrepare
        ? "A conflicting Prepare was dropped; honest digest still formed a certificate."
        : "No conflicting Prepare observed.",
      evidence: [
        { source: "SovereignLattice", field: "conflicting_prepare", value: String(args.lattice.conflictingPrepare) },
        { source: "SovereignLattice", field: "prepared_certificate", value: String(args.lattice.preparedCertificate) },
      ],
    });
  }

  // Checks (Sentinel-shaped)
  if (args.cross) {
    checks.push({
      ...
      evidence: [
        { source: "CrossNodeReport", field: "agreement_ratio", value: args.cross.agreementRatio.toFixed(2) },
        { source: "CrossNodeReport", field: "common_block_hash", value: args.cross.commonHash ?? "UNAVAILABLE" },
      ],
    });
  }
  if (args.replay) {
    checks.push({
      ...
      status: args.replay.match ? "PASS" : "FAIL",
      severity: "CRITICAL",
      message: args.replay.match
        ? "Local replay matches the observed post-state hash."
        : "Execution divergence: expected and observed state hashes differ.",
      source: "Replay",
      evidence: [
        { source: "Replay", field: "match", value: String(args.replay.match) },
      ],
    });
  }
  checks.push({
    ...
    status: args.authorized ? "PASS" : "FAIL",
    severity: "CRITICAL",
    message: args.authorized
      ? "Event originated under a present capability."
      : "Capability absent — no Rholang process was deployed.",
    source: "QuantumOS",
    evidence: [
  ...
  const claims: Claim[] = [
    {
      layer: "quantumos",
      statement: args.authorized
        ? "A room event was originated under a capability that authorizes the lemma."
        : "Origin was refused — the required capability was not present.",
      status: args.authorized ? "PASS" : "FAIL",
    ...
    {
      layer: "rspace",
      statement: args.execution
        ? `ρ-calculus reducer performed ${args.execution.comms} COMM steps.`
    ...
    {
      layer: "sentinel",
      statement: args.cross
        ? `Observed agreement ${args.cross.agreeingNodes}/${args.cross.targetCount} (ratio ${args.cross.agreementRatio.toFixed(2)}).`
        : "No observations — no block.",
    ...
    {
      layer: "lattice",
      statement: args.lattice
        ? `CommittedCertificate ${args.lattice.committedCertificate ? "present" : "absent"} for digest ${shortHex(args.lattice.digest)}.`
        : "No lattice input.",
  ...
  if (failInv && failInv.status !== "PASS") {
    ...
    if (args.mutation === "node-c-lied") {
      ...
      witness = {
        ...
        field: "block_hash",
        impact:
          "Sentinel cross-node hash agreement fails. Lattice may still form a quorum on the honest digest — these are different claims.",
        verification: "FAIL",
      };
    }
    if (args.mutation === "drop-capability") {
      witness = {
        layer: "quantumos",
    ...
    if (args.mutation === "tamper-trace" && args.replay?.firstDivergence) {
      witness = {
        layer: "rspace",
  ...
  if (args.blocks[0]) {
    ...
    if (args.cross) {
      why.push(
        `Sentinel observed ${args.cross.agreeingNodes}/${args.cross.targetCount} nodes on hash ${shortHex(args.cross.commonHash ?? "")}. This is observed agreement, not stake-weighted finality.`,
      );
    }
    if (args.lattice) {
      why.push(
        `Sovereign Lattice formed ${args.lattice.committedCertificate ? "a" : "no"} CommittedCertificate (Q=${args.lattice.quorum}) on the honest digest. Independent of Sentinel's observation set.`,
      );
    }
  } else {
    why.push("No block exists. The compiler stopped at QuantumOS — authorization failed — so downstream layers have no object to observe.");
  }
