...
function pretty(p: Proc): string {
  switch (p.k) {
    case "0":
      return "Nil";
    case "|":
      return p.ps.map(pretty).join(" | ");
    case "new":
      return `new ${p.ns.join(", ")} in { ${pretty(p.p)} }`;
    case "send":
      return `${showName(p.ch)}!(${p.data.map(showG).join(", ")})`;
    case "for":
      return `for (${p.binds.map((b) => `@${b}`).join(", ")} <- ${showName(p.ch)}) { ${pretty(p.p)} }`;
  }
}
...
export function reduce(source: string, proc: Proc): Execution {
    ...
    for (let ci = 0; ci < consumes.length; ci++) {
      ...
      while (active.length) {
        ...
        if (q.k === "send") produces.push({ id: nid("p"), ch: chKey(q.ch, env), data: q.data });
          ...
          consumes.push({ id: nid("c"), ch: chKey(q.ch, env), binds: q.binds, body: q.p });
  ...
  steps.push({
    ...
    description: stuck
      ? "no further COMM; leftover produce/consume"
      : `normal form — ${comms} COMM reduction(s)`,
    produces: produces.length,
    consumes: consumes.length,
  });
  ...
  const traceHash = hexPrefixed(
    digest(["trace", ...steps.map((s) => `${s.rule}:${s.description}`)]),
  );
...
function subst(p: Proc, env: Map<string, string>): Proc {
  ...
  switch (p.k) {
    case "0":
      return p;
    case "|":
      return { k: "|", ps: p.ps.map((q) => subst(q, env)) };
    case "new":
      return { k: "new", ns: p.ns, p: subst(p.p, env) };
    case "send":
      return { k: "send", ch: name(p.ch), data: p.data };
    case "for":
      return { k: "for", ch: name(p.ch), binds: p.binds, p: subst(p.p, env) };
  }
}

function substGround(p: Proc, binds: string[], data: Ground[]): Proc {
  // Continuations after COMM are typically Nil in this subset.
  void binds;
  void data;
  return p;
}
...
${lines.join(" |\n")}
  ...
  const sends: Proc[] = verbs.map((v) => ({
    k: "send" as const,
    ch: { k: "var" as const, id: "pool" },
    data: [v],
  }));
  const recvs: Proc[] = verbs.map(() => ({
    k: "for" as const,
    ch: { k: "var" as const, id: "pool" },
    binds: ["ack"],
    p: { k: "0" as const },
  ...
  return {
    source,
    proc: { k: "new", ns: ["pool"], p: { k: "|", ps: [...sends, ...recvs] } },
  };
}
