...
Each CI matrix worker checks out the **actual corresponding source repository** at the exact SHA, executes the named test with Node or Cargo, hashes its full stdout/stderr, and records the test command, source identity and exit code. The separate bundler checks all four entries are present, successful, at the expected commits, and have matching log bytes before publishing `four-repo-bundle.json`. A failed worker cannot become a four-of-four PASS report.
